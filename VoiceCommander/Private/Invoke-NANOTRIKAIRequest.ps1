function Invoke-NANOTRIKAIRequest {
    [CmdletBinding()]
    param (
        [Parameter(Mandatory)]
        [string]$InputText,
...
"@
    ...
    $Body = @{
        ...
        'temperature' = 0.3
        'max_tokens' = 150
        'provider' = 'nanotrik.ai'
    } | ConvertTo-Json

    try {
        Write-Verbose "Sending request to NANOTRIK.AI API..."
        $Response = Invoke-RestMethod -Uri 'https://api.nanotrik.ai/v1/chat/completions' `
                                    -Method Post `
                                    -Headers $Headers `
                                    -Body $Body
